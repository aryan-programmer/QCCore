...
namespace QCCore
{
	...
	class BasicString : public std::basic_string<T>
	{
		...
		forceinline BasicString( const std::basic_string_view<T>& v );
	public:
	...
	template<typename T>
	class BasicStringRef : public std::basic_string_view<T>
	{
	...
	template<typename T> forceinline BasicString<T>::BasicString( const std::basic_string_view<T>& v ) :base_t( v ) { }
}